...
let records=loadJSON(KEY,[]), users=loadJSON(USER_KEY,[]), currentDetailId=null, formImages=[];
...
function newForm(){$('rfiForm').reset();formImages=[];renderImagePreviews();$('recordId').value='';$('dateRaised').value=dateISO();$('status').value='Open';$('priority').value='Normal';$('formTitle').textContent='New RFI';showView('form')}
...
function editForm(r){Object.keys(r).forEach(k=>{const el=$(k);if(el&&!['images'].includes(k))el.value=r[k]??''});formImages=Array.isArray(r.images)?structuredClone(r.images):[];renderImagePreviews();$('formTitle').textContent='Edit RFI';showView('form')}

async function handleImageInput(e){
 const files=[...(e.target.files||[])];e.target.value='';
 if(!files.length)return;
 const remaining=Math.max(0,4-formImages.length);
 if(!remaining){alert('Maximum 4 images per RFI.');return}
 const selected=files.slice(0,remaining);
 for(const file of selected){
   if(!/^image\/(jpeg|png|webp)$/.test(file.type)){continue}
   try{const compressed=await compressImage(file);formImages.push({id:crypto.randomUUID(),name:file.name,dataUrl:compressed,addedAt:new Date().toISOString()})}
   catch(err){console.error(err);alert(`Failed to process ${file.name}`)}
 }
 if(files.length>remaining)alert(`Only ${remaining} additional image(s) could be added. Maximum 4 images per RFI.`);
 renderImagePreviews();
}
function compressImage(file){return new Promise((resolve,reject)=>{const reader=new FileReader();reader.onerror=reject;reader.onload=()=>{const img=new Image();img.onerror=reject;img.onload=()=>{const max=1100,scale=Math.min(1,max/Math.max(img.width,img.height));const w=Math.max(1,Math.round(img.width*scale)),h=Math.max(1,Math.round(img.height*scale));const canvas=document.createElement('canvas');canvas.width=w;canvas.height=h;const ctx=canvas.getContext('2d');ctx.fillStyle='#fff';ctx.fillRect(0,0,w,h);ctx.drawImage(img,0,0,w,h);resolve(canvas.toDataURL('image/jpeg',0.68))};img.src=reader.result};reader.readAsDataURL(file)})}
function renderImagePreviews(){const box=$('imagePreviewGrid');if(!box)return;if(!formImages.length){box.innerHTML='<div class="image-empty">No image attached.</div>';return}box.innerHTML=formImages.map((im,i)=>`<div class="image-card"><img src="${im.dataUrl}" alt="RFI attachment ${i+1}"><button type="button" class="remove-image" onclick="removeFormImage('${im.id}')" title="Remove image">×</button><div class="image-meta">${i+1}. ${esc(im.name||'Image')}</div></div>`).join('')}
window.removeFormImage=function(id){formImages=formImages.filter(x=>x.id!==id);renderImagePreviews()}
function imageAppendix(images,rfiNo){if(!Array.isArray(images)||!images.length)return '';return `<section class="pdf-image-appendix"><div class="pdf-logo-wrap"><img class="pdf-logo" src="gets-logo.png" alt="GeTs Architects logo"></div><div class="pdf-image-title">IMAGE ATTACHMENTS — ${esc(rfiNo||'RFI')}</div><div class="pdf-image-grid">${images.map((im,i)=>`<div class="pdf-image-item"><img src="${im.dataUrl}" alt="Attachment ${i+1}"><div class="pdf-image-caption">Image ${i+1}${im.name?' — '+esc(im.name):''}</div></div>`).join('')}</div><div class="pdf-footer">GeTs Architects&nbsp;&nbsp;|&nbsp;&nbsp;Request for Information (RFI) — Image Attachment</div></section>`}
...
function backupPayload(reason='manual'){return {app:'GeTs RFI Control',version:'v5',reason,backupAt:new Date().toISOString(),records,users,driveSettings:loadJSON(DRIVE_KEY,{})}}
